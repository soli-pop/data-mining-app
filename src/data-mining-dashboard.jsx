    ...
    <div ref={outerRef} style={{ position:"relative", overflow:"hidden", borderBottom:isLast?"none":"1px solid #f1f5f9", animation:"rowIn .25s ease both" }}>
      ...
      <div ref={slideRef} style={{ background:"#fff", willChange:"transform", touchAction:"pan-y" }}>
        ...
        <div onClick={()=>onToggle(expanded?null:row.id)}
          style={{ display:"grid", gridTemplateColumns:"minmax(0,1.8fr) minmax(0,1.4fr) minmax(0,1.1fr) 20px", padding:"13px 16px", alignItems:"center", cursor:"pointer", userSelect:"none" }}>
          ...
          {/* Spend + spend label + country */}
          <div>
            <div style={{ fontWeight:700, fontSize:"0.85rem", color:"#0f172a", marginBottom:3 }}>{fmtM(row.spend)}</div>
            <Badge label={spnL+" Spend"} color={SPN_C[spnL]||"#64748b"}/>
            <div style={{ fontSize:"0.67rem", color:"#64748b", fontWeight:600, marginTop:3 }}>{row.country}</div>
          </div>
            ...
            <div style={{ background:"#f8fafc", borderRadius:12, padding:12, marginBottom:10, border:"1px solid #f0f4f8" }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                {[
                  ...
                  {lbl:"Purchase / Category", raw:row.purchase,  badge:catL,  color:CAT_C[catL]||"#6366f1"},
                  {lbl:"Amt. Spent",raw:fmtM(row.spend),badge:spnL+" Spend", color:SPN_C[spnL]||"#64748b"},
                  {lbl:"Country / Region",raw:row.country,badge:cntL,         color:REG_C[cntL]||"#3b82f6"},
...
const DesktopRow = ({ row, isLast, isGuest, onEdit, onDelete, idx }) => {
  ...
  return (
    <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{ display:"grid", gridTemplateColumns:"minmax(0,2fr) 0.9fr 1.15fr 1.15fr 1.1fr 1.15fr 130px", padding:"13px 20px", alignItems:"center", borderBottom:isLast?"none":"1px solid #f1f5f9", background:hov?"#fafbff":"#fff", transition:"background .15s", animation:`rowIn .2s ease ${idx*0.04}s both` }}>
      {/* Name */}
      <div style={{ minWidth:0 }}>
        <div style={{ fontWeight:700, fontSize:"0.9rem", color:"#0f172a", marginBottom:4, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{row.name||"—"}</div>
      ...
      {/* Amt. Spent */}
      <div><div style={{ fontSize:"0.85rem", color:"#334155", fontWeight:600 }}>{fmtM(row.spend)}</div><div style={{ marginTop:3 }}><Badge label={spnL+" Spend"} color={SPN_C[spnL]||"#64748b"}/></div></div>
      {/* Country / Region */}
      <div><div style={{ fontSize:"0.85rem", color:"#334155", fontWeight:600 }}>{row.country}</div><div style={{ marginTop:3 }}><Badge label={cntL} color={REG_C[cntL]||"#3b82f6"}/></div></div>
      {/* Actions */}
            ...
            {tab==="table" ? (
              <>
                ...
                <div style={{ background:"#fff", borderRadius:16, boxShadow:"0 2px 10px rgba(0,0,0,0.07)", overflow:"hidden", marginBottom:14 }}>
                  {/* Header */}
                  <div style={{ display:"grid", gridTemplateColumns:wide?"minmax(0,2fr) 0.9fr 1.15fr 1.15fr 1.1fr 1.15fr 130px":"minmax(0,1.8fr) minmax(0,1.4fr) minmax(0,1.1fr) 20px", padding:wide?"12px 20px":"10px 14px", background:"#f8fafc", borderBottom:"1px solid #f1f5f9" }}>
                    {(wide
                      ? [
                      ...
                      : [
                          {h:"Name",  s:"Category · Region"},
                          {h:"Age",   s:"Income Level"},
                          {h:"Spend", s:"Spend Level · Country"},
                          {h:"",      s:""},
                        ]
                    ).map((c,i)=>(
