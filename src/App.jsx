...
const DesktopRow = ({ row, isLast, isGuest, onEdit, onDelete, idx }) => {
  ...
  return (
    <div
      ...
      style={{
        display:"grid",
        gridTemplateColumns:"minmax(140px,2fr) 90px 140px 120px 140px 130px 150px",
        alignItems:"center",
        padding:"14px 16px",
...
const Login = ({ onLogin }) => {
  ...
  const isLandscape = useWide(); // reuse — true when viewport >= 768px wide (landscape tablet/desktop too)

  // also detect pure mobile landscape (short height + wide)
  const [mobileLandscape, setMobileLandscape] = useState(
    () => window.innerWidth > window.innerHeight && window.innerHeight < 500
  );
  useEffect(()=>{
    const h = () => setMobileLandscape(window.innerWidth > window.innerHeight && window.innerHeight < 500);
    window.addEventListener("resize", h);
    return () => window.removeEventListener("resize", h);
  },[]);
  ...
  const T=({id,label})=>(
    <button onClick={()=>{setTab(id);setErr("");}} style={{ flex:1, padding:"9px", border:"none", borderRadius:9, cursor:"pointer", background:tab===id?"#3b82f6":"transparent", color:tab===id?"#fff":"#64748b", fontWeight:700, fontSize:"0.82rem", fontFamily:"inherit", transition:"all .15s" }}>{label}</button>
  );
  ...
  const FormContent = (
    <div style={{ display:"flex", flexDirection:"column", flex:1, minHeight:0 }}>
      ...
      <button onClick={tab==="login"?doLogin:doSignup} disabled={loading}
        style={{ width:"100%", padding:"11px", borderRadius:10, border:"none", background:loading?"#93c5fd":"#3b82f6", color:"#fff", fontWeight:700, fontSize:"0.88rem", cursor:loading?"not-allowed":"pointer", fontFamily:"inherit", boxShadow:"0 3px 12px rgba(59,130,246,0.35)", display:"flex", alignItems:"center", justifyContent:"center", gap:7, transition:"background .2s" }}>
        {loading
  ...
  if (splitLayout) {
    ...
    return (
      <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#eff6ff 0%,#f0f4f8 100%)", display:"flex", alignItems:"stretch", justifyContent:"center" }}>
        ...
        <div style={{ display:"flex", width:"100%", maxWidth:860, margin:"auto", borderRadius:20, overflow:"hidden", boxShadow:"0 24px 60px rgba(0,0,0,0.15)", animation:"fadeUp .4s ease" }}>

          {/* Left — brand panel */}
          <div style={{ flex:"0 0 42%", background:"linear-gradient(155deg,#1d4ed8 0%,#3b82f6 60%,#60a5fa 100%)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"40px 32px", position:"relative", overflow:"hidden" }}>
            {/* Decorative circles */}
            ...
            <div style={{ position:"relative", zIndex:1, textAlign:"center" }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:8, marginTop:28 }}>
                {[
                  ...
                  <div key={f.text} style={{ display:"flex", alignItems:"center", gap:10, background:"rgba(255,255,255,0.12)", borderRadius:10, padding:"8px 14px" }}>
          ...
          {/* Right — form panel */}
          <div style={{ flex:1, background:"#fff", display:"flex", flexDirection:"column", justifyContent:"center", padding:"32px 36px", overflowY:"auto" }}>
  ...
  return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#eff6ff 0%,#f0f4f8 100%)", display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
      ...
      <div style={{ width:"100%", maxWidth:400, animation:"fadeUp .4s ease" }}>
        ...
        <div style={{ background:"linear-gradient(135deg,#2563eb,#3b82f6)", borderRadius:"20px 20px 0 0", padding:"28px 28px 24px", textAlign:"center" }}>
        ...
        <div style={{ background:"#fff", borderRadius:"0 0 20px 20px", padding:24, boxShadow:"0 12px 40px rgba(0,0,0,0.1)" }}>
          {FormContent}
        </div>
      </div>
    </div>
...
export default function App() {
  ...
  if (!user) return (
    <div style={{ fontFamily:"'DM Sans','Inter','Segoe UI',sans-serif" }}>
      ...
      <Login onLogin={u=>{ setUser(u); push(u.role==="guest" ? "Welcome, Guest!" : `Welcome back, ${u.name}!`,"ok"); }}/>
    </div>
  );
            ...
            {tab==="table" ? (
              <>
                ...
                <div style={{ background:"#fff", borderRadius:16, boxShadow:"0 2px 10px rgba(0,0,0,0.07)", overflow:"hidden", marginBottom:14, border:"1.5px solid #d1d9e6" }}>
                  {/* Scrollable inner wrapper for narrower desktops */}
                  <div style={{ overflowX: wide ? "auto" : "hidden", minWidth:0 }}>
                  {/* Header */}
                  <div style={{ display:"grid", gridTemplateColumns:wide?"minmax(140px,2fr) 90px 140px 120px 140px 130px 150px":"minmax(0,2fr) minmax(0,1.5fr) minmax(0,1.2fr) 22px", gap:wide?8:8, padding:wide?"11px 16px":"10px 14px", background:"#eef1f7", borderBottom:"2px solid #d1d9e6" }}>
                    {(wide
                      ? [
