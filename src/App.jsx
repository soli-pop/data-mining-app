...
const Badge = ({ label, color }) => (
  <span style={{ background:color+"18", color, border:"1px solid "+color+"35", fontSize:"0.6rem", fontWeight:700, padding:"2px 8px", borderRadius:20, whiteSpace:"nowrap", display:"inline-block", letterSpacing:"0.02em" }}>
    {label}
  </span>
);
...
const SwipeRow = ({ row, isLast, isGuest, onEdit, onDelete, expanded, onToggle }) => {
  ...
  const ageL=getAgeLabel(row.age), incL=getIncomeLabel(row.income), catL=getCat(row.purchase), cntL=getRegion(row.country);
  ...
  return (
    <div ref={outerRef} style={{ position:"relative", overflow:"hidden", borderBottom:isLast?"none":"1px solid #f1f5f9", animation:"rowIn .25s ease both" }}>
      ...
      <div ref={bgRef} style={{ position:"absolute", inset:0, background:"#dc2626", display:"flex", alignItems:"center", paddingLeft:22, opacity:0, pointerEvents:"none" }}>
        <div ref={iconWrap} style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:3, opacity:0, transform:"scale(0.4)" }}>
          ...
          <span style={{ fontSize:"0.58rem", fontWeight:800, color:"#fff", letterSpacing:"0.08em" }}>DELETE</span>
        </div>
      </div>

      <div ref={slideRef} style={{ background:"#fff", willChange:"transform", touchAction:"pan-y" }}>
        {/* Main row */}
        <div onClick={()=>onToggle(expanded?null:row.id)}
          style={{ display:"grid", gridTemplateColumns:"minmax(0,2fr) minmax(0,1.6fr) minmax(0,1.1fr) 20px", padding:"13px 16px", alignItems:"center", cursor:"pointer", userSelect:"none" }}>
          <div style={{ minWidth:0 }}>
            <div style={{ fontWeight:700, fontSize:"0.9rem", color:"#0f172a", marginBottom:4, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{row.name||"—"}</div>
          ...
          <div>
            <div style={{ fontWeight:700, fontSize:"0.85rem", color:AGE_C[ageL]||"#1e293b", marginBottom:4 }}>
              {row.age} <span style={{ fontWeight:500, fontSize:"0.75rem" }}>({ageL})</span>
            </div>
            <Badge label={incL+" Income"} color={INC_C[incL]||"#64748b"}/>
          </div>
          <div>
            <div style={{ fontWeight:700, fontSize:"0.85rem", color:"#0f172a", marginBottom:2 }}>{fmtM(row.spend)}</div>
        ...
        {/* Expanded panel */}
        <div style={{ overflow:"hidden", maxHeight:expanded?"300px":"0", transition:"max-height .3s cubic-bezier(0.4,0,0.2,1)" }}>
          <div style={{ padding:"0 14px 14px" }}>
            <div style={{ background:"#f8fafc", borderRadius:12, padding:12, marginBottom:10, border:"1px solid #f0f4f8" }}>
              <div style={{ display:"flex", alignItems:"center", gap:5, marginBottom:10 }}>
                ...
                <span style={{ fontSize:"0.6rem", fontWeight:800, color:"#94a3b8", letterSpacing:"0.07em", textTransform:"uppercase" }}>Identifiers</span>
              </div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                {[
                  ...
                  <div key={lbl} style={{ background:"#fff", borderRadius:9, padding:"9px 10px", boxShadow:"0 1px 3px rgba(0,0,0,0.05)" }}>
                    <div style={{ fontSize:"0.57rem", fontWeight:800, color:"#94a3b8", letterSpacing:"0.06em", marginBottom:3 }}>{lbl.toUpperCase()}</div>
                    <div style={{ fontSize:"0.82rem", fontWeight:700, color:"#1e293b", marginBottom:5 }}>{raw}</div>
            ...
            <div style={{ display:"flex", gap:8 }}>
              <button onClick={e=>{e.stopPropagation();onToggle(null);onEdit(row);}}
                style={{ flex:1, padding:"10px", borderRadius:10, border:"1.5px solid #bfdbfe", background:"#eff6ff", color:"#1d4ed8", fontWeight:700, fontSize:"0.8rem", cursor:"pointer", fontFamily:"inherit", display:"flex", alignItems:"center", justifyContent:"center", gap:6 }}>
                <Icon name="edit" size={14} color="#1d4ed8"/> Edit
              </button>
              {!isGuest && (
                <button onClick={e=>{e.stopPropagation();onToggle(null);onDelete(row);}}
                  style={{ flex:1, padding:"10px", borderRadius:10, border:"1.5px solid #fecaca", background:"#fff7f7", color:"#dc2626", fontWeight:700, fontSize:"0.8rem", cursor:"pointer", fontFamily:"inherit", display:"flex", alignItems:"center", justifyContent:"center", gap:6 }}>
                  <Icon name="trash" size={14} color="#dc2626"/> Delete
                </button>
              )}
              <button onClick={e=>{e.stopPropagation();onToggle(null);}}
                style={{ padding:"10px 14px", borderRadius:10, border:"1.5px solid #e2e8f0", background:"#f8fafc", cursor:"pointer", fontFamily:"inherit", display:"flex", alignItems:"center", justifyContent:"center" }}>
            ...
            {!isGuest && (
              <div style={{ marginTop:9, display:"flex", alignItems:"center", justifyContent:"center", gap:5 }}>
                ...
                <span style={{ fontSize:"0.63rem", color:"#cbd5e1", fontWeight:600 }}>Swipe row right to quick-delete</span>
              </div>
            )}
          </div>
        </div>
...
const DesktopRow = ({ row, isLast, isGuest, onEdit, onDelete, idx }) => {
  ...
  return (
    <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{ display:"grid", gridTemplateColumns:"minmax(0,2.2fr) 1fr 1.2fr 1.2fr 1.1fr 1.1fr 130px", padding:"13px 20px", alignItems:"center", borderBottom:isLast?"none":"1px solid #f1f5f9", background:hov?"#fafbff":"#fff", transition:"background .15s", animation:`rowIn .2s ease ${idx*0.04}s both` }}>
      <div style={{ minWidth:0 }}>
        <div style={{ fontWeight:700, fontSize:"0.9rem", color:"#0f172a", marginBottom:4, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{row.name||"—"}</div>
        <div style={{ display:"flex", gap:4 }}><Badge label={catL} color={CAT_C[catL]||"#6366f1"}/><Badge label={cntL} color={REG_C[cntL]||"#3b82f6"}/></div>
      </div>
      <div><span style={{ fontWeight:700, fontSize:"0.85rem", color:AGE_C[ageL]||"#1e293b" }}>{row.age}</span><div style={{ marginTop:3 }}><Badge label={ageL} color={AGE_C[ageL]||"#6366f1"}/></div></div>
      <div><div style={{ fontSize:"0.85rem", color:"#334155", fontWeight:600 }}>{fmtM(row.income)}</div><div style={{ marginTop:3 }}><Badge label={incL+" Income"} color={INC_C[incL]||"#64748b"}/></div></div>
      <div><div style={{ fontSize:"0.85rem", color:"#334155", fontWeight:600 }}>{row.purchase}</div><div style={{ marginTop:3 }}><Badge label={catL} color={CAT_C[catL]||"#6366f1"}/></div></div>
      <div><div style={{ fontSize:"0.85rem", color:"#334155", fontWeight:600 }}>{fmtM(row.spend)}</div></div>
      <div><div style={{ fontSize:"0.85rem", color:"#334155", fontWeight:600 }}>{row.country}</div></div>
      <div style={{ display:"flex", gap:6, justifyContent:"flex-end" }}>
        <button onClick={()=>onEdit(row)} style={{ display:"flex", alignItems:"center", gap:5, padding:"6px 12px", borderRadius:8, border:"1.5px solid #bfdbfe", background:"#eff6ff", color:"#1d4ed8", fontWeight:600, fontSize:"0.75rem", cursor:"pointer", fontFamily:"inherit" }}>
          <Icon name="edit" size={12} color="#1d4ed8"/> Edit
        </button>
        {!isGuest && (
          <button onClick={()=>onDelete(row)} style={{ display:"flex", alignItems:"center", gap:5, padding:"6px 12px", borderRadius:8, border:"1.5px solid #fecaca", background:"#fff7f7", color:"#dc2626", fontWeight:600, fontSize:"0.75rem", cursor:"pointer", fontFamily:"inherit" }}>
...
export default function App() {
  ...
  const handleDelete = useCallback(row=>{ setTarget(row); setModal("delete"); },[]);
  ...
  const doDel  = () => { const n=target?.name; setData(p=>p.filter(r=>r.id!==target.id)); setModal(null); setTarget(null); push(`${n} deleted`,"del"); };
  ...
  return (
    <div style={{ fontFamily:"'DM Sans','Inter','Segoe UI',sans-serif", background:"#f0f4f8", minHeight:"100vh" }}>
      <style>{`*{box-sizing:border-box}body{margin:0}input,select,button{font-family:inherit}::-webkit-scrollbar{width:4px}::-webkit-scrollbar-thumb{background:#e2e8f0;border-radius:4px}`}</style>
      ...
      <div style={{ display:"flex", minHeight:"100vh" }}>
        ...
        <div style={{ flex:1, display:"flex", flexDirection:"column", minWidth:0 }}>
          ...
          <div style={{ padding:wide?"24px":"14px", flex:1, maxWidth:wide?"1200px":"100%", width:"100%", margin:"0 auto", alignSelf:"stretch" }}>
            ...
            {/* Search + Filters */}
            <div style={{ display:"flex", gap:10, marginBottom:14, flexWrap:wide?"nowrap":"wrap", alignItems:"center" }}>
              <div style={{ position:"relative", flex:1, minWidth:wide?"260px":"100%" }}>
                ...
                <input value={search} onChange={e=>{setSearch(e.target.value);setPage(1);}} placeholder="Search by name, age, product, country…"
                  style={{ width:"100%", padding:"11px 14px 11px 40px", borderRadius:12, border:"1.5px solid #e2e8f0", background:"#fff", fontSize:"0.88rem", outline:"none", boxShadow:"0 2px 8px rgba(0,0,0,0.06)", color:"#1e293b", transition:"border-color .2s" }}
                  onFocus={e=>e.target.style.borderColor="#3b82f6"} onBlur={e=>e.target.style.borderColor="#e2e8f0"}/>
              </div>
            ...
            {tab==="table" ? (
              <>
                ...
                {/* Table */}
                <div style={{ background:"#fff", borderRadius:16, boxShadow:"0 2px 10px rgba(0,0,0,0.07)", overflow:"hidden", marginBottom:14 }}>
                  {/* Header */}
                  <div style={{ display:"grid", gridTemplateColumns:wide?"minmax(0,2.2fr) 1fr 1.2fr 1.2fr 1.1fr 1.1fr 130px":"minmax(0,2fr) minmax(0,1.6fr) minmax(0,1.1fr) 20px", padding:wide?"12px 20px":"10px 14px", background:"#f8fafc", borderBottom:"1px solid #f1f5f9" }}>
                    {(wide
                      ? [{h:"Name",s:"Product · Region"},{h:"Age",s:"Label"},{h:"Income",s:"Level"},{h:"Purchase",s:"Category"},{h:"Spend",s:""},{h:"Country",s:""},{h:"Actions",s:""}]
                      : [{h:"Name",s:"Product · Region"},{h:"Age",s:"Income Level"},{h:"Spend",s:"Country"},{h:"",s:""}]
                    ).map((c,i)=>(
                      <div key={i} style={{ textAlign:wide&&i===6?"right":"left" }}>
                        <div style={{ fontSize:"0.72rem", fontWeight:800, color:"#475569" }}>{c.h}</div>
                  ...
                  {/* Pagination */}
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:7, padding:13, borderTop:"1px solid #f1f5f9" }}>
      ...
      <Modal open={modal==="delete"} title="Delete Record"  onClose={()=>{setModal(null);setTarget(null);}} center width={360}>
